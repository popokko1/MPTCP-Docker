...
#include "/src/mptcp/scratch/second_kojima_re.h"
//#define PLOT
#define RAND_GAP
      ...
      if (m_state != CLOSED)
        {
          ...
          NS_LOG_UNCOND(endtime << "["<< m_node->GetId() << "] CloseMultipathConnection -> MPTCP connection is closed {" << this << "}, m_state: " << TcpStateName[m_state] << " -> CLOSED" << " CurrentSubflow (" << (int)currentSublow << ") SubflowsSize: " <<subflows.size());
          m_state = CLOSED;
          NotifyNormalClose();
...
MpTcpSocketBase::OpenCWND(uint8_t sFlowIdx, uint32_t ackedBytes)
{
  //std::cout << "waimaru" << vari << std::endl;
  NS_LOG_FUNCTION(this << (int) sFlowIdx << ackedBytes);
  Ptr<MpTcpSubFlow> sFlow = subflows[sFlowIdx];
 // printf("%d!?!?!?!?", Npacket);
  double adder = 0;
  ...
  uint32_t ssthresh = sFlow->ssthresh;
  uint32_t mss = sFlow->MSS;
  uint32_t RTT = sFlow->lastMeasuredRtt.GetMilliSeconds();
  uint32_t min_RTT = sFlow->min_RTT;
  ...
  if (cwnd < ssthresh)
   {
    if(gsam==1)//whether entering the fase of smooth transition or not
     {
      if(sFlow->min_RTT==0)
        { 
          sFlow->Smooth_RTT = RTT;
          sFlow->min_RTT = RTT; 
          sFlow->cwnd += sFlow->MSS;
          sFlow->K = ((buffer * mss * 8 + speed * 1000 * RTT)/(2*speed*1000) + RTT)/2;
         // uint32_t dn = (50 * mss * 8 + 5120 * RTT)/(2 * 5120);
        }
      else
        { 
          sFlow->Smooth_RTT = 7 * sFlow->Smooth_RTT / 8 + RTT/8; 
          if(RTT < min_RTT)
            {
//              min_RTT = RTT;
              sFlow->min_RTT = RTT; 
              sFlow->K = ((buffer * mss * 8 + speed * 1000 * RTT)/(2*speed*1000) + RTT)/2;
             // sFlow->K = (10 * mss + 5000 * min_RTT)/(4*5000);
            }
          if(sFlow->Smooth_RTT >= sFlow->K)
            {
              printf("SSS");
              double del_pro = (RTT - sFlow->K);
              double del = del_pro/RTT;
              double p = 0;
              double cw = sFlow->cwnd;             
              double rev = static_cast<double>(mss) / cw;
              p = pow(rev, del);
              double p2 = p * mss;
              sFlow->cwnd += static_cast<double>(p2);
              uint32_t nd2 = sFlow->cwnd;
              printf("%d", nd2);
            }
          else
            {
